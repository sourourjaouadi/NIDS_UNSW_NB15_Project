...
export const Header = ({ onUploadClick, backendOnline }: HeaderProps) => {
    ...
    <header className="sticky top-0 z-40 border-b border-white/6 bg-[#0D1117]/70 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-4 lg:flex">
          ...
          >
            <span className={`h-2 w-2 rounded-full ${backendOnline ? "bg-emerald-400" : "bg-amber-300"}`} />
            {backendOnline ? "Backend connected" : "Waiting for backend"}
          </div>
          <button
            type="button"
